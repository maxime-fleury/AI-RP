...
class BreezeForConditionalGeneration(BreezePreTrainedModel, BreezeGenerationMixin):
    ...
    def __init__(self, config):
        ...
        if getattr(config, "text_encoder_config", None) is not None:
            ...
            # Fall back to eager when flash_attn is not installed so the model
            # still runs on any CUDA machine (slower, but works). Checked even
            # when the checkpoint explicitly requested flash_attention_2.
            if text_encoder_attn_implementation == "flash_attention_2":
                try:
