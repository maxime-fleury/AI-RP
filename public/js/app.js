import { api, apiFetch, apiForm, uploadFiles, setToken, readSseStream, fileToPngDataUrl } from "./api.js";
import { el, esc, toast, actionToast, openModal, confirmModal, closeAllModals, field, ICONS, fmtTime, fmtAge } from "./ui.js";
import { renderChat } from "./chat.js";
import { guidedWizard, newGameWizard } from "./wizard.js";
// re-export: chat.js loads the wizard lazily through the app module
export { guidedWizard, newGameWizard };
...
function fireShortcut(k) {
  import("./chat.js").then((m) => m.chatShortcut(k)).catch(() => {});
}
...
async function renderSettings() {
  ...
  container.append(el("div", { class: "card", style: { padding: "18px 22px" } },
    ...
    el("div", { style: { marginTop: "18px", borderTop: "1px solid var(--border)", paddingTop: "14px" } },
      el("div", { class: "row", style: { justifyContent: "space-between" } },
        el("span", { class: "sec-sub" }, "📊 Métriques RP"),
        el("button", { class: "btn btn-ghost btn-sm", title: "Actualiser", onclick: loadMetricsBox, "aria-label": "Actualiser les métriques" }, "↻"),
      ),
      el("div", { id: "metrics-box", style: { marginTop: "10px" } }),
    ),
  ));
...
// Réglages → Métriques : Phase 8 consumer — drift/regen/edit rates per profile,
// read from the diagnostics payload (data/metrics.jsonl folded server-side).
async function loadMetricsBox() {
  const box = document.getElementById("metrics-box");
  if (!box) return;
  try {
    const d = await api("/api/diagnostics");
    const m = d.metrics || {};
    const line = (...parts) => el("div", { class: "storage-line" }, ...parts);
    const chips = (obj) => {
      const entries = Object.entries(obj || {});
      return entries.length ? entries.map(([k, v]) => `${k}: ${v}`).join(" · ") : "—";
    };
    if (!m.turns) {
      box.replaceChildren(line("Aucun tour enregistré — les métriques apparaissent après quelques tours de jeu."));
      return;
    }
    const n = (x) => (typeof x === "number" ? x.toFixed(1) : String(x));
    box.replaceChildren(
      line(el("b", {}, `${m.turns} tours`), ` · garde-fou : ${m.driftTriggers} (${n(m.driftRate)}%)`, ` · OOC : ${m.oocTurns} · éditions : ${m.edits} · régénérations : ${m.regenerates}`),
      line("Moyennes : ", `${n(m.avgSecs)} s/tour · ${m.avgPromptTokens} tokens de prompt · fenêtre ${n(m.windowDays)} j`),
      line("Comportements : ", chips(m.byBehavior)),
      line("Contextes : ", chips(m.byMode)),
      line("Focus : ", chips(m.byFocus)),
    );
  } catch {
    box.replaceChildren();
  }
...
export function checkbox(key, checked, labelText) {
